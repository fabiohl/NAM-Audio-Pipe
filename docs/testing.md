...
- **Recording Worker Guard & Observable Join ([`src/recording/guard_test.rs`](../src/recording/guard_test.rs)):** Validates the RAII teardown of `nam-recording-io` (F-RB-009 / T3.5): premature-drop cleanup with bounded join, bounded retry on `push_stream_stop`, join-timeout detection, formal inspection of the worker `Result` and panic payloads, and the ordered StreamStop → producer drop → join shutdown. T5.1 (F-RB-024, D1) additions: the D1 exit-code policy (`RecordingWorkerOutcome::exit_code()`: `Success` → 0; `SuccessWithLoss`/`Failed`/`Panicked`/`TimedOut` → 1) and the loss classification — a clean join with `OVERRUN_COUNT`/`OVERRUN_FRAMES_COUNT` > 0 becomes `SuccessWithLoss { blocks, frames }` (`shutdown_reports_success_with_loss_when_overruns_were_counted`), while zero overruns keep `Success` unchanged (`shutdown_with_zero_overruns_stays_success`). **Exit-code policy (D1):** a recording that lost audio blocks to ring overruns is a **non-zero** exit (`SuccessWithLoss`) with an explicit message — `frames_capturados == frames_enfileirados + frames_perdidos` is reconcilable and never silent (see `main.rs` mapping).
- **Recording transport rollback coverage (T5.4, F-RB-025):** the `Inline` branch behind `RECORDING_POOL_TRANSPORT` (`src/recording/transport.rs`) is **structurally unreachable in production** — the const is hard-coded `true` and nothing flips it at runtime. It has **no coverage on the production surface**, only dedicated unit tests (`transport.rs` `tests` module and the `RecordingReceiver::Inline` arms in `guard_test.rs`/`disk.rs` unit paths). It is kept intentionally as the documented T4.1 rollback; plan: remove it entirely in a future release if it is never needed (no behavior change when removed — see `RECORDING_POOL_TRANSPORT` doc).

### 2.2 Integration Test Suites ([`tests/`](../tests/))

- **CLI Black-Box Smoke Tests ([`tests/e2e_cli.rs`](../tests/e2e_cli.rs)):** Executes the compiled binary (`CARGO_BIN_EXE_nam-audio-pipe`) using `std::process::Command` to verify CLI help screens, diagnostic output formatting, `--gate on|off` validation (rejecting invalid options like `--gate invalid`), and robust error handling for invalid or out-of-range options. G-RB-003 / T6.1 negative acceptances: `-b 100`, `-b 1` and `-b 16384` exit non-zero with an explanatory `Argument error` on stderr *before any PipeWire connection* (validated by asserting the message is absent of PipeWire output).
...
- **Service Resilience Harness ([`tests/service_resilience.rs`](../tests/service_resilience.rs), ER-4 / T4.6):** Certifies the ER-4 gates. Three live subprocess acceptances (Phase 3, `#[ignore]`d):
...
| **PHASE2** | RT-Safety heap-audit (zero-alloc)                              | `cargo test --features "testing heap-audit" --release` (lib unit gates `cabsim_swap::heap_audit_tests` + RT paths) + `tests/swap_stress.rs` heap-audit battery; oracle: `assert_eq!(get_alloc_count(), 0)` and shell greps for `swap_soak_heap_audit                                                                                        | heap_audit_`             |
...
| **Dist Binary Smoke (ER-5)**         | `--profile dist` (stripped, LTO fat; `panic = "unwind"` so F-RB-020 containment works in the shipped artifact) binary exits 0 on `--diagnose`/`--help`, emits diagnostic bundle, no crash artifacts                                                                                                                                                                                              | `tests/distribution_qa.rs` / `utils/build-release.sh`                                                                                                                                       |
| **Buffer Size Domain (G-RB-003)**    | `--buffer-size` domain strictly `{0} ∪ {2^k                                                                                                                                                                                                                                                                                    | 16 <= 2^k <= 8192}`; out-of-domain values (`1, 2, 8, 15, 63, 100, 500, 8193, 16384, 65536, u32::MAX`) rejected with the typed`BufferSizeError` before any PipeWire connection or allocation |
