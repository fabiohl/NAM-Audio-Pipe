...
pub fn setup_capture_stream<'c>(
    ...
    let capture_listener = capture_stream
        ...
        .process(move |stream: &pw::stream::Stream, _info| {
            ...
            // — and raises the fatal `RT_STATUS_PANIC_CAPTURED` latch for the
            // main control loop to run the ordered teardown.
            rt_callback::run_rt_callback_body(
                ...
                std::panic::AssertUnwindSafe(|| {
                    ...
                    if !state.thread_configured {
                        rt_setup::configure_realtime_thread(
                            target_cpu,
                            rt_status_for_process.clone(),
                        );
                    ...
                    if (state.frame_count.wrapping_sub(1) & 0xF) == 0 {
                        let elapsed_nanos =
                            rt_status_for_process.dsp_cycle_time.load(Ordering::Relaxed);
                        if elapsed_nanos > 0 {
                            let n_samples =
                                rt_status_for_process.last_n_samples.load(Ordering::Relaxed) as u64;
                            if n_samples > 0 && current_host_rate > 0 {
                                let budget_ns =
                                    n_samples * 1_000_000_000 / current_host_rate as u64;
